...
from cmp_core.services.ec2 import create_ec2, delete_ec2, get_ec2, list_ec2, update_ec2
...
async def api_create_ec2(
    *,
    ...
    dto: Ec2Create = Body(..., description="Параметри EC2 у вигляді JSON"),
    db: AsyncSession = Depends(get_db),
    user=Depends(require_project_member(RoleName.devops)),
):
    """
    Створити EC2 інстанс у проєкті. Мінімальна роль DevOps.
    """
    return await create_ec2(db, project_id, dto, user.id)
...
async def api_delete_ec2(
    ...
    """
    Видалити EC2 інстанс. Мінімальна роль DevOps.
    """
    await delete_ec2(db, project_id, name, user.id)
    return None
